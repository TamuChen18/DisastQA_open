...
MODEL_CONFIGS = {
    "llama-3-8b": {
        ...
        "base_max_new_tokens": 128,  # 基础token数，会根据difficulty调整
        "generation_config": {
    ...
    "phi-2": {
        ...
        "base_max_new_tokens": 150,  # 小模型，减少token数
        "torch_dtype": torch.float16,
        "device_map": "auto",
...
class LocalOEModelEvaluator:
    ...
    def generate_answer(self, query: str, context: List[str], difficulty: str = "medium") -> str:
...
Answer:"""
                ...
                # 根据difficulty调整max_new_tokens
                base_tokens = self.model_config["base_max_new_tokens"]
                difficulty_tokens = difficulty_config["max_new_tokens"]
                # 优先使用difficulty配置，但不超过模型能力上限
                adjusted_tokens = min(difficulty_tokens, 500)  # 设置500作为安全上限
                    ...
                    try:
                        ...
                        if self.setting == "mix" and lookup_key in self.golden_lookup:
                            ...
                            result = {
                                ...
                                'difficulty': item['difficulty']  # 添加difficulty
                            }
                ...
                for item in batch:
                    try:
                        response = self.generate_answer(
                            ...
                            difficulty=item['difficulty']  # 传递difficulty
                        )
                        single_result = {
                            'question': item['question'],
                            'correct_answer': item['correct_answer'],
    ...
    def generate_answer_batch(self, queries: List[str], contexts: List[List[str]], difficulties: List[str]) -> List[str]:
        ...
        try:
            ...
            for query, context, difficulty in zip(queries, contexts, difficulties):
                ...
                # 统一为0-shot prompt with difficulty-aware length constraint
                if context_str:
                    prompt = f"""Passage: {context_str}
...
Answer:"""
            ...
            # 生成回答 - 使用最大的difficulty token数来确保覆盖所有情况
            print("\nGenerating batch responses...")
            with torch.no_grad():
                gen_config = self.model_config["generation_config"]
                
                # 使用基础token数，因为批处理中可能有不同difficulty
                base_tokens = self.model_config["base_max_new_tokens"]
