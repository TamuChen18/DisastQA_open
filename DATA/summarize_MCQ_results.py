...
def load_existing_results():
    ...
    results_file = "/home/shared/RAG_DATA/DATA/MCQ_category_analysis_results.json"
...
def save_summary_results(summary_data, all_intents, all_events, all_results):
    ...
    header.extend([f'{intent} Acc' for intent in all_intents])
    header.extend([f'{event} Acc' for event in all_events])
    
    # Save to CSV
    csv_file = "/home/shared/RAG_DATA/DATA/MCQ_summary_by_intent_event.csv"
    with open(csv_file, 'w', encoding='utf-8') as f:
        ...
        for row in summary_data:
            csv_row = [row['Model'], row['Dataset']]
            for intent in all_intents:
                csv_row.append(row[f'{intent}_acc'])
            for event in all_events:
                csv_row.append(row[f'{event}_acc'])
            f.write(','.join(csv_row) + '\n')
    
    print(f"✅ Summary table saved to: {csv_file}")
    
    # Save to JSON
    json_file = "/home/shared/RAG_DATA/DATA/MCQ_summary_by_intent_event.json"
...
def main():
    ...
    # Save results
    save_summary_results(summary_data, all_intents, all_events, all_results)
