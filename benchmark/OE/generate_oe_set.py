...
class TestSetGenerator:
    ...
    def _process_single_query(self, query: Dict):
        ...
        for retry in range(max_retries):
            ...
            # Randomly select a score=3 passage as reference
            score_3_passages = query["passages_by_score"].get("3", [])
            if not score_3_passages:
                print("No score=3 passages found")
                return
            reference_passage = random.choice(score_3_passages)
            print(f"Selected reference passage from {len(score_3_passages)} score=3 passages")
            ...
            # Generate open-ended questions and answers using GPT-4o-mini
            print("Generating open-ended question and answer...")
            gpt40_oe = self.generate_with_retry(self.generate_open_ended,
                query["user_query"], 
                reference_passage,  # use the selected score=3 passage as reference
                "gpt-4o-mini",
                query["general_type"]
            )
            
            if gpt40_oe is None or "content" not in gpt40_oe:
                print("Failed to generate open-ended question and answer")
            ...
            gpt40_oe_quality = self.assess_question_quality(
                gpt40_oe["content"]["question"],
                gpt40_oe["content"]["answer"],
                "gpt-4o-mini"
            )
            
            # Create base test case (no passage)
            base_case = {
                "original_query": query["user_query"],
                "setting": "base",
                "passage": None,  # no passage for base case
                "metadata": {
                    "query_id": query["filename"],
                    "passage_id": "",
                    "quality_score": 0,
                    "original_quality": original_quality,
                    "generated_quality": gpt40_oe_quality,
                    "difficulty_level": "easy" if original_quality["difficulty"] <= 3 else "hard"
                },
                "open_ended": {
                    "gpt40": {
                        "content": gpt40_oe["content"]
                    }
                }
            }
            ...
            # Create golden test case (using score=3 passage)
            golden_case = {
                "original_query": query["user_query"],
                "passage": reference_passage,  # use the score=3 passage
                "setting": "golden",
                "metadata": {
                    "query_id": query["filename"],
                    "passage_id": "",
            ...
            self.test_set.append(golden_case)
            print("Added golden test case")

            # Create mix test case (30% chance to use low-score passage)
            mix_passage = self._get_mix_passage(query, reference_passage)
            mix_case = {
                "original_query": query["user_query"],
                "passage": mix_passage,
                "setting": "mix",
                "metadata": {
                    "query_id": query["filename"],
                    "passage_id": "",
                    "quality_score": 1.5,  # average quality
                    "original_quality": original_quality,
                    "generated_quality": gpt40_oe_quality,
                    "difficulty_level": "easy" if original_quality["difficulty"] <= 3 else "hard",
                    "replacement_info": {
                        "is_replaced": mix_passage != reference_passage,
                        "original_score": 3.0,
                        "new_score": self._get_passage_score(mix_passage, query)
                    }
                },
                "open_ended": {
                    "gpt40": {
            ...
            self.test_set.append(mix_case)
            print("Added mix test case")
    ...
    def generate_open_ended(self, user_query: str, positive_doc: str, model: str, general_type: str) -> Dict:
        ...
        Given the following **user query** and **reference document** (the gold evidence), create an improved open-ended question and provide a comprehensive answer:
        ...
        try:
            ...
            try:
                ...
                raise Exception("Failed to generate a valid open-ended question")
    ...
    def _get_mix_passage(self, query: Dict, reference_passage: str) -> str:
        """Get mix passage (30% chance to use low-score passage)"""
        if random.random() < 0.3:  # 30% probability to use low-score passage
            # 1. collect all low-score passages
            low_score_passages = {}
            for score in ["0", "1", "2"]:
                passages = query["passages_by_score"].get(score, [])
                if passages:
                    low_score_passages[score] = passages
            
            if low_score_passages:
                # 2. equally probability select score (0, 1, or 2)
                selected_score = random.choice(list(low_score_passages.keys()))
                # 3. randomly select passage from selected score
                selected_passage = random.choice(low_score_passages[selected_score])
                return selected_passage
        
        # 70% probability to use reference passage
        return reference_passage

    def _get_passage_score(self, passage: str, query: Dict) -> float:
        """Get the score of a passage based on passages_by_score"""
        for score in ["3", "2", "1", "0"]:
            if score in query["passages_by_score"]:
                for p in query["passages_by_score"][score]:
                    if p == passage:
                        return float(score)
        
        # If not found, return default score
        return 1.5
    ...
    def _generate_test_case(self, query: Dict, passages_by_score: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # 5. generate open-ended question
            with self.api_semaphore:
                oe_question = self.generate_open_ended(
                    user_query,
                    best_passage,
                    "gpt-4o-mini",
                    general_type
            ...
            if not oe_question:
                print(f"Warning: Failed to generate open-ended question")
                return None
...
def main(num_cases: Optional[int] = None):
    ...
    try:
        ...
        output_dir = '/home/shared/RAG_DATA/benchmark/OE/generated_QA'
