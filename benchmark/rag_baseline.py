...
Methods:
1. keyword_only: BM25 retrieval (3,5,8 docs) → Local model generation
2. vector_only: Vector retrieval (3,5,8 docs) → Local model generation  
3. hybrid_only: BM25+Vector retrieval (3,5,8 docs) → Local model generation
        ...
        for test_set_path in test_set_paths:
            ...
            for method in methods:
                ...
                for config_idx, config in enumerate(param_configs):
                    ...
                    output_file = os.path.join(output_dir, f"{test_set_name}_{method}_{config_name}_{self.local_model_name}_results.json")
                    with open(output_file, 'w', encoding='utf-8') as f:
                        json.dump(results, f, indent=2, ensure_ascii=False)
                    print(f"Results saved to: {output_file}")
        
        # Save combined results
        combined_output_file = os.path.join(output_dir, f"all_baseline_{self.local_model_name}_results.json")
